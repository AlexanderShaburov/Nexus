...
**When Nexus itself is the problem** (a gate blocks a compliant turn, a spec contradicts the work, a hook misbehaves, or something works well enough to protect): in a project with Nexus installed, do not patch the host's copy of a Nexus file. Write a feedback note, `knowledge/feedback/feedback--nexus--<slug>.md` (`type: feedback`, `scope: nexus`, `kind: bug|wish|praise`, the sections *What happened* / *What is proposed* / *Attachment*), commit it as ordinary vault writeback, and run `python3 tools/nexus-update.py feedback push`. The note reaches the Nexus repository through the local mailbox and the fix returns with the next update. Contract: `knowledge/specs/spec--system--feedback-channel.md`. In the Nexus template repository itself there is no host copy to protect: fix the template.
...
- `.claude/hooks/nexus-update-check.py` (`SessionStart` only) — in a project with a `.nexus/installed.json` baseline, asks the Nexus repository at most once a day, with one `git ls-remote` bounded to 5 s, whether a newer `v*` tag exists, and says so in one line if it does. Silent otherwise and silent on every failure. It never applies anything: updating is `python3 tools/nexus-update.py plan` then `apply --apply`, run by the operator from a plain terminal. In this template, where no baseline exists, the same hook instead reports how many feedback notes from host projects are waiting in the local mailbox (`python3 tools/nexus-update.py feedback list` to read them). Contracts: `knowledge/specs/spec--system--nexus-update.md` §7, `knowledge/specs/spec--system--feedback-channel.md` §4.

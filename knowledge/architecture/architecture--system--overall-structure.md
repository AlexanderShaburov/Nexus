...
- `tools/nexus-update.py` — the updater. In this phase: `manifest generate|verify` (template), `baseline` and `status` (host). Stdlib only; reuses the frontmatter parser of `tools/validate-vault.py`. Delivered to hosts as part of `tools/`.
...
- `.nexus/update-check.json` — cache of the last upstream version check (not yet written; reserved, gitignored).

...
| 3 | `apply` + core freeze | backups, temp+rename writes, group order, post-apply validation, `--restore`, report-only conflicts; the core-freeze rule in `nexus-tool-gate.py`, `.nexus/unlock.txt`, spec and architecture mirror (§7.2a) | **Done 2026-09-22.** Selftest T9 covers dry-run, clean rows written, blocking and customized rows untouched, backups, baseline rewrite per spec §8, no-op re-run, `--restore` per path, the live-session guard and the `hooks-merge` writer. Gate tested by piping JSON (report B15–B17): core edit denied, unlocked / project / template edits allowed; the new gate was built in a side file and swapped in only after the tests. Rehearsal on the throwaway host: the guard fired on a fresh `state.json` (exit 6); `apply --apply` wrote six files including both hooks; post-apply validation then failed on a broken link the rehearsal itself had planted in the host's index (exit 5, files in place, backup named), which is the intended behaviour. `spec--system--nexus-update.md` promoted to binding. |
| 4 | notifier | `.claude/hooks/nexus-update-check.py`, registration in `.claude/settings.json`, entry in the architecture doc §2d and the implementation report | **Done 2026-09-22.** Piped-JSON rows B18–B20: silent without a baseline, when up to date against the real GitHub remote, when throttled with a bogus URL (no network call), when unreachable with a stale record, when upstream has no tags, when the baseline is corrupt, and in the template; one line when a bare clone carrying `v9.9.9` is the upstream, again when throttled. Registered on `SessionStart` only, after the bootstrap hook, and only after the tests passed. Architecture §2e, spec §7, CLAUDE.md, index and report mirrored. |
| 5 | retrofit | rehearsal on a template copy, then Liquid_Nexus, then others; runbook `runbook--system--nexus-update-retrofit.md` | each host has a committed `installed.json` and a clean `plan`. **Re-sequenced 2026-09-22:** done with the installer of Phase 6 (`up` detects "Nexus present, no baseline" and runs `baseline --guess` + `apply`), on the operator's machine. **Liquid_Nexus retrofitted 2026-09-22** via `curl … nexus.py \| python3 - --apply --restore tools/validate-vault.py` from `v1.2.0`: 39 identical / 10 customized / 2 absent against `v1.0.0`, 10 files written, baseline at 1.2.0, validation clean; 7 conflicts + 3 customizations left alone for review, all three diffs inspected so far showed only stale Nexus wording, no host-specific content. Two observations: (1) `--guess` scored every candidate equally (39/10) because the host's forks differ from every ref, so ties fall to the tag; harmless for the baseline, but a tie should be printed as such in a later refinement; (2) when `tools/validate-vault.py` is itself in conflict, post-apply validation runs the host's **old** validator, so the first `--apply` must carry `--restore tools/validate-vault.py`; documented in the guide's Part 6a would be the right place. **Completed 2026-09-22 19:18:** seven conflicting core files restored with `up --apply --restore …`, `/hooks` reloaded (8 hooks), `status`: 55 unchanged, 2 customized (the two specs carrying the host's `plan_status` contribution, ported upstream in `bae06aa` and expected to converge at the next release). The core freeze is now active in Liquid_Nexus. Other hosts: same procedure when the operator reaches them |
| 6 | installer (§10a, operator decision 2026-09-22) | `install` and `up` subcommands in `tools/nexus-update.py` (auto-detect: no Nexus → install; Nexus without baseline → retrofit; baseline → update); `nexus.py` single-file bootstrap at the template root (cache clone, highest tag, run the updater from the tag); clean-tree requirement or explicit flag; final instructions for `/hooks` and `/project-ingest` | **Done 2026-09-22.** Selftest T11 covers all of it. Rehearsal against the real repository showed the expected dependency: `v1.1.0`'s updater has no `up` and its validator still treats links into `plans/` as errors, so the bootstrap works from the **next release** onward (`nexus.py` now says so when pointed at an older tag). Two supporting changes: `LK002` warning for dangling links into `plans/` / `sessions/`, and the glossary cites the rename ADR by path instead of linking it, so a host with only delivered documents validates clean. |

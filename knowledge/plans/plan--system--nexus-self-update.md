...
Plan status: **in_progress** (approved by the operator on 2026-09-22 with the decisions in §12; Phases 1–4 implemented the same day, see §10. Phase 5 (retrofit of Liquid_Nexus) not started; the feedback channel plan can now start, since it borrows this notifier and the cache. `v1.0.0` is tagged on `ac83f1c` and pushed; it is the first release the updater can target.)
...
| 6 | installer (§10a, operator decision 2026-09-22) | `install` and `up` subcommands in `tools/nexus-update.py` (auto-detect: no Nexus → install; Nexus without baseline → retrofit; baseline → update); `nexus.py` single-file bootstrap at the template root (cache clone, highest tag, run the updater from the tag); clean-tree requirement or explicit flag; final instructions for `/hooks` and `/project-ingest` | selftest T11: install into an empty directory, install into a project with its own `CLAUDE.md` / `settings.json` / `.gitignore` (project content preserved), retrofit of a copy of an older Nexus without baseline (customizations reported, nothing overwritten); guide, spec and architecture mirrored. Requires release `v1.1.0` first, since the installer targets tags |
